...
for i in range(iterations):
    ...
    #  cells[cells > 0] -= 1.3
    cells -= 1/9*cells # stats.norm.rvs(0,np.sqrt(2*0.01*dt),N*N).reshape(N,N)
    cells[cells < 2.5] = 0

    cells[np.where(mask_up <= -9.5)] = 10
    cells[np.where(mask_dw <= -9.5)] = 10
    cells[np.where(mask_LR <= -9.5)] = 10
    cells[np.where(mask_RL <= -9.5)] = 10
